// "use client";
// import Image from "next/image";
// import React, { useEffect } from "react";
// import AOS from "aos";
// import "aos/dist/aos.css";

// const ImpactSection = () => {
//   useEffect(() => {
//     AOS.init({
//       duration: 1000, // Animation duration
//       once: true, // Trigger animations only once
//     });
//   }, []);

//   return (
//     <section className="py-16">
//       <div className=" lg:mx-auto mx-0 lg:px-4 px-2">
//         {/* Section Header */}
//         <div className="text-center mb-12" data-aos="fade-up">
//           <h2 className="lg:text-[35px] text-[27px] tracking-tight font-bold mb-4">
//             The Impact of the Victhaw <br className="lg:block hidden" /> Official Unity Cup
//           </h2>
//           <p className="text-[16px] tracking-tight text-[#6B6F76] m-auto lg:w-[590px] w-auto">
//             Sports have the power to transform lives, build character, and unite
//             people from all walks of life. My goal is to create a platform where
//             athletes can showcase their talents, embrace discipline, and inspire
//             the next generation. This is more than just competition, it’s about
//             fostering resilience, teamwork, and a lasting impact on the
//             community.
//             <br />– <strong>Adeniji Gbolabo Victor, Founder</strong>
//           </p>
//         </div>

//         {/* Victor's Photo
//         <div className="relative h-96 rounded-lg overflow-hidden shadow-lg mb-12" data-aos="fade-up">
//           <Image
//             src="/victor-tournament.jpg" // Replace with your image path
//             alt="Victor at the Tournament"
//             layout="fill"
//             objectFit="cover"
//             className="rounded-lg"
//           />
//         </div> */}

//         {/* Content Grid */}
//         <div
//           className="grid grid-cols-1 lg:grid-cols-4 gap-8 tracking-tight lg:px-[50px] px-[5px]"
//           data-aos="fade-up"
//         >
//           {/* Impact Card 1: Community Engagement */}
//           <div className="bg-[#FAFAFA] p-6 rounded-lg shadow-md text-center">
//             <div className="text-[#C81E23] text-4xl font-bold mb-4">16+</div>
//             <h3 className="text-xl font-semibold mb-2">Teams</h3>
//             <p className="text-[#6B6F76] w-auto">
//               Uniting talented football teams from different backgrounds,
//               fostering competition, teamwork, and a shared passion for the
//               game.
//             </p>
//           </div>

//           {/* Impact Card 2: Talent Development */}
//           <div className="bg-[#FAFAFA] p-6 rounded-lg shadow-md text-center">
//             <div className="text-[#C81E23] text-4xl font-bold mb-4">250+</div>
//             <h3 className="text-xl font-semibold mb-2">Players</h3>
//             <p className="text-[#6B6F76] w-auto">
//               Offering a platform for athletes to shine, develop their skills,
//               and take their football careers to the next level.
//             </p>
//           </div>

//           {/* Impact Card 3: Health and Fitness */}
//           <div className="bg-[#FAFAFA] p-6 rounded-lg shadow-md text-center">
//             <div className="text-[#C81E23] text-4xl font-bold mb-4">1K+</div>
//             <h3 className="text-xl font-semibold mb-2">Spectators</h3>
//             <p className="text-[#6B6F76]">
//               Bringing the community together to experience the thrill of the
//               game while promoting fitness, engagement, and a love for sports.
//             </p>
//           </div>

//           {/* Impact Card 4: Economic Benefits */}
//           <div className="bg-[#FAFAFA] p-6 rounded-lg shadow-md text-center">
//             <div className="text-[#C81E23] text-4xl font-bold mb-4">10+</div>
//             <h3 className="text-xl font-semibold mb-2">Sponsors</h3>
//             <p className="text-[#6B6F76] w-auto">
//               Providing businesses with valuable exposure, brand visibility, and
//               a chance to support grassroots sports development.
//             </p>
//           </div>
//         </div>

//         {/* Testimonials */}
//         <div className="mt-16" data-aos="fade-up">
//           <h3 className="text-2xl font-bold text-[#C81E23] text-center mb-8">
//             What People Are Expecting
//           </h3>
//           <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:px-[50px] px-[5px]">
//             {/* Anticipation 1 */}
//             <div className="bg-[#FAFAFA] p-6 rounded-lg shadow-md">
//               <p className="text-[#6B6F76] italic">
//                 "I can't wait for the Victhaw Official Unity Cup! It's going to
//                 be an amazing opportunity to showcase our skills and connect
//                 with other talented players."
//               </p>
//               <div className="mt-4 flex items-center">
//                 <div className="w-12 h-12 bg-gray-200 rounded-full mr-4"></div>
//                 <div>
//                   <p className="font-semibold ">Kolapo James</p>
//                   <p className="text-sm text-gray-500">Player, Storm FC</p>
//                 </div>
//               </div>
//             </div>

//             {/* Anticipation 2 */}
//             <div className="bg-[#FAFAFA] p-6 rounded-lg shadow-md">
//               <p className="text-[#6B6F76] italic">
//                 "Sponsoring this event is an exciting opportunity! We are
//                 looking forward to being part of something that will bring the
//                 community together in a meaningful way."
//               </p>
//               <div className="mt-4 flex items-center">
//                 <div className="w-12 h-12 bg-gray-200 rounded-full mr-4"></div>
//                 <div>
//                   <p className="font-semibold">Abubakar Sheikh</p>
//                   <p className="text-sm text-gray-500">
//                     Sponsor Representative
//                   </p>
//                 </div>
//               </div>
//             </div>

//             {/* Anticipation 3 */}
//             <div className="bg-[#FAFAFA] p-6 rounded-lg shadow-md">
//               <p className="text-[#6B6F76] italic">
//                 "I'm so excited for the first edition of the tournament! It’s
//                 going to be an unforgettable experience, and I can't wait to
//                 witness the energy and passion on the field."
//               </p>
//               <div className="mt-4 flex items-center">
//                 <div className="w-12 h-12 bg-gray-200 rounded-full mr-4"></div>
//                 <div>
//                   <p className="font-semibold ">Isaiah Johnson</p>
//                   <p className="text-sm text-gray-500">Spectator</p>
//                 </div>
//               </div>
//             </div>
//           </div>
//         </div>

//         {/* Call to Action */}
//         <div className="text-center mt-16" data-aos="fade-up">
//           <a
//             href="/register" 
//             className="bg-[#C81E23] text-white px-8 py-4 rounded-lg hover:bg-red-900 transition duration-300"
//           >
//             Be Part of the Impact
//           </a>
//         </div>
//       </div>
//     </section>
//   );
// };

// export default ImpactSection;





"use client";
import Image from "next/image";
import React, { useEffect } from "react";
import AOS from "aos";
import "aos/dist/aos.css";

const ImpactSection = () => {
  useEffect(() => {
    AOS.init({
      duration: 1000,
      once: true,
    });
  }, []);

  // Function to generate avatar fallback
  const getAvatarFallback = (name) => {
    const initials = name.split(' ').map(n => n[0]).join('').toUpperCase();
    return (
      <div className="w-12 h-12 bg-gray-200 rounded-full mr-4 flex items-center justify-center">
        <span className="text-gray-600 font-semibold">{initials}</span>
      </div>
    );
  };

  return (
    <section className="py-16">
      <div className="lg:mx-auto mx-0 lg:px-4 px-2">
        {/* Section Header */}
        <div className="text-center mb-12" data-aos="fade-up">
          <h2 className="lg:text-[35px] text-[27px] tracking-tight font-bold mb-4">
            The Impact of Victhaw Unity Cup 1.0
          </h2>
          <p className="text-[16px] tracking-tight text-[#6B6F76] m-auto lg:w-[590px] w-auto">
            The inaugural Unity Cup successfully transformed lives, built character, and united 
            people from all walks of life. We created a platform where athletes showcased their 
            talents, embraced discipline, and inspired their communities. This was more than 
            just competition—it was about fostering resilience, teamwork, and lasting impact.
            <br />– <strong>Adeniji Gbolabo Victor, Founder</strong>
          </p>
        </div>

        {/* Content Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 tracking-tight lg:px-[50px] px-[5px]" data-aos="fade-up">
          {/* Impact Card 1 */}
          <div className="bg-[#FAFAFA] p-6 rounded-lg shadow-md text-center">
            <div className="text-[#C81E23] text-4xl font-bold mb-4">16+</div>
            <h3 className="text-xl font-semibold mb-2">Teams</h3>
            <p className="text-[#6B6F76] w-auto">
              United football teams from different backgrounds, fostering competition, teamwork, and passion for the game.
            </p>
          </div>

          {/* Impact Card 2 */}
          <div className="bg-[#FAFAFA] p-6 rounded-lg shadow-md text-center">
            <div className="text-[#C81E23] text-4xl font-bold mb-4">250+</div>
            <h3 className="text-xl font-semibold mb-2">Players</h3>
            <p className="text-[#6B6F76] w-auto">
              Showcased their skills, developed their abilities, and created unforgettable moments.
            </p>
          </div>

          {/* Impact Card 3 */}
          <div className="bg-[#FAFAFA] p-6 rounded-lg shadow-md text-center">
            <div className="text-[#C81E23] text-4xl font-bold mb-4">1K+</div>
            <h3 className="text-xl font-semibold mb-2">Spectators</h3>
            <p className="text-[#6B6F76]">
              Experienced the thrill of the game and celebrated community spirit.
            </p>
          </div>

          {/* Impact Card 4 */}
          <div className="bg-[#FAFAFA] p-6 rounded-lg shadow-md text-center">
            <div className="text-[#C81E23] text-4xl font-bold mb-4">10+</div>
            <h3 className="text-xl font-semibold mb-2">Sponsors</h3>
            <p className="text-[#6B6F76] w-auto">
              Supported grassroots sports development and connected with passionate audiences.
            </p>
          </div>
        </div>

        {/* Testimonials */}
        <div className="mt-16" data-aos="fade-up">
          <h3 className="text-2xl font-bold text-[#C81E23] text-center mb-8">
            What People Are Saying
          </h3>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:px-[50px] px-[5px]">
            {/* Testimonial 1 */}
            <div className="bg-[#FAFAFA] p-6 rounded-lg shadow-md">
              <p className="text-[#6B6F76] italic">
                "The Unity Cup 1.0 was an incredible experience! We got to showcase our skills against top competition while building lasting connections."
              </p>
              <div className="mt-4 flex items-center">
                {getAvatarFallback("Kolapo James")}
                <div>
                  <p className="font-semibold">K. James</p>
                  <p className="text-sm text-gray-500">Player</p>
                </div>
              </div>
            </div>

            {/* Testimonial 2 */}
            <div className="bg-[#FAFAFA] p-6 rounded-lg shadow-md">
              <p className="text-[#6B6F76] italic">
                "Sponsoring this event was rewarding. The energy and community spirit exceeded our expectations."
              </p>
              <div className="mt-4 flex items-center">
                {getAvatarFallback("Abubakar Sheikh")}
                <div>
                  <p className="font-semibold">A. Sheikh</p>
                  <p className="text-sm text-gray-500">Sponsor Representative</p>
                </div>
              </div>
            </div>

            {/* Testimonial 3 */}
            <div className="bg-[#FAFAFA] p-6 rounded-lg shadow-md">
              <p className="text-[#6B6F76] italic">
                "The tournament atmosphere was electric! I've never seen our community come together like this for sports."
              </p>
              <div className="mt-4 flex items-center">
                {getAvatarFallback("Isaiah Johnson")}
                <div>
                  <p className="font-semibold">I. Johnson</p>
                  <p className="text-sm text-gray-500">Spectator</p>
                </div>
              </div>
            </div>
          </div>
        </div>

        {/* Call to Action */}
        <div className="text-center mt-16" data-aos="fade-up">
          <a
            href="/tournament/results"
            className="bg-[#C81E23] text-white px-8 py-4 rounded-lg hover:bg-red-900 transition duration-300"
          >
            View Tournament Highlights
          </a>
        </div>
      </div>
    </section>
  );
};

export default ImpactSection;