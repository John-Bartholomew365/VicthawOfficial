import Link from "next/link";
import React from "react";

const TournamentHighlight = () => {
  return (
    <div className="my-16">
      <div>
        <h1 className="lg:text-5xl text-3xl font-bold leading-tight text-center tracking-tight mb-10">
          The Next Chapter of <br /> Something Greater
        </h1>
      </div>

      <div className="lg:flex lg:flex-row flex-col items-center justify-center lg:ml-[70px] ml-0">
        {/* Left Image */}
        <div className="w-full lg:w-1/2 flex justify-center lg:justify-start">
          <img
            src="/use3.jpeg"
            alt="Fans celebrating"
            className="w-full lg:h-[500px] max-w-[468px] h-auto rounded-md object-cover"
          />
        </div>

        {/* Right Content */}
        <div className="w-full lg:w-1/2 mt-8 lg:mt-0">
          <h2 className="text-xl lg:text-2xl font-semibold leading-tight tracking-tight mb-6 lg:w-[440px] w-auto">
            Unity Cup 2.0 is bigger, bolder, and better than ever!
          </h2>
          <ul className="ml-5 space-y-4 mb-7 tracking-tight">
            <li className="flex items-start gap-3 lg:w-[410px] w-auto">
              <span className="text-red-600">●</span>
              <div className="flex flex-col">
                <div>
                  <strong>Expanded 8-Team Format:</strong>
                </div>
                <div>
                  <p className="text-black">
                    <span className="text-[#6B6F76]">
                      Eight teams split into Groups A and B, with the top 2 from
                      each group advancing to the semi-finals and a grand final.
                    </span>
                  </p>
                </div>
              </div>
            </li>
            <li className="flex items-start gap-3 lg:w-[410px] w-auto">
              <span className="text-red-600">●</span>
              <div className="flex flex-col">
                <div>
                  <strong>Proven Platform for All Ages (25-50):</strong>
                </div>
                <div>
                  <p className="text-black">
                    <span className="text-[#6B6F76]">
                      Unity Cup 1.0 successfully showcased talent across
                      generations. Version 2.0 is even more competitive and
                      inclusive.
                    </span>
                  </p>
                </div>
              </div>
            </li>
            <li className="flex items-start gap-3 lg:w-[410px] w-auto">
              <span className="text-red-600">●</span>
              <div className="flex flex-col">
                <div>
                  <strong>Professional Execution:</strong>
                </div>
                <div>
                  <p className="text-black">
                    <span className="text-[#6B6F76]">
                      Certified referees, top-notch officials, and seamless
                      coordination across every matchday.
                    </span>
                  </p>
                </div>
              </div>
            </li>
            <li className="flex items-start gap-3 lg:w-[410px] w-auto">
              <span className="text-red-600">●</span>
              <div className="flex flex-col">
                <div>
                  <strong>Rewards That Motivate:</strong>
                </div>
                <div>
                  <p className="text-black">
                    <span className="text-[#6B6F76]">
                      Compete for trophies, medals, cash prizes, and individual
                      accolades including MVP, Best Goalkeeper, and Top Scorer.
                    </span>
                  </p>
                </div>
              </div>
            </li>
          </ul>
          <div className="flex flex-col sm:flex-row gap-4">
            <Link className="ml-0 lg:ml-6" href="/tournament/schedule">
              <button className="bg-[#C81E23] flex justify-center items-center text-center py-[10px] px-[20px] text-white rounded-[6px] hover:bg-white hover:text-black hover:border-[#C81E23] hover:border transition-all lg:w-fit w-[80%] lg:mx-0 mx-auto">
                View Schedule
              </button>
            </Link>
            <Link href="/tournament/team">
              <button className="bg-white border border-[#C81E23] flex justify-center items-center text-center py-[10px] px-[20px] text-[#C81E23] rounded-[6px] hover:bg-[#C81E23] hover:text-white transition-all lg:w-fit w-[80%] lg:mx-0 mx-auto">
                Meet the Teams
              </button>
            </Link>
          </div>
        </div>
      </div>
    </div>
  );
};

export default TournamentHighlight;
